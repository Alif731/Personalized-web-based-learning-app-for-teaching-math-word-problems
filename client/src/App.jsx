...
import { getProblem, submitAnswer, getStatus } from './services/api';
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            {!feedback ? (
               problem && !problem.complete ? (
                <QuestionCard problem={problem} onSubmit={handleAnswerSubmit} />
                 ...
                 <div className="text-center p-12 text-gray-500">Loading your challenge...</div>
               )
            ) : (
              <div className={`p-8 rounded-2xl shadow-xl text-center ${feedback.isCorrect ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
                <div className="text-6xl mb-4">{feedback.isCorrect ? '🌟' : '❌'}</div>
                <h2 className="text-3xl font-bold mb-4">{feedback.isCorrect ? 'Awesome!' : 'Oops!'}</h2>
                <p className="text-xl mb-6">
                  {feedback.isCorrect ? feedback.explanation : `The correct answer was: ${feedback.correctAnswer}`}
                </p>
                <button 
                  onClick={handleNext}
          ...
          <div className="md:col-span-1">
             ...
             <div className="mt-8 bg-yellow-100 p-4 rounded-xl border-l-4 border-yellow-400">
                <h4 className="font-bold text-yellow-800 mb-2">Zone of Proximal Development</h4>
                <p className="text-sm text-yellow-700">
                  The system is currently targeting: 
                  <span className="font-bold block mt-1">{status?.zpdNodes.join(', ') || 'Loading...'}</span>
                </p>
             </div>
          </div>
        </div>
