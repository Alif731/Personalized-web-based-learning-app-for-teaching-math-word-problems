...
if (isProduction) {
  ...
  if (jwtSecret.length < 32 || weakSecrets.has(jwtSecret.toLowerCase())) {
    throw new Error('JWT_SECRET is missing or weak. Set a strong production secret (32+ chars).');
  }
}

const shouldSeedDemoUser = process.env.NODE_ENV !== 'production' && process.env.SEED_DEMO_USER !== 'false';
